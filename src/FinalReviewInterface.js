...
function FinalReviewInterface({
  ...
  const { processedPostsLog, spotifyPlaylistId, spotifyPlaylistName } =
    firebasePlaylistObj;
  ...
  useEffect(() => {
    if (submissionSuccess !== null && submissionSuccess) {
      ...
      mockSleep(2500).then(() =>
        history.push(
          `/stats?spotifyPlaylistId=${spotifyPlaylistId}&firebaseMetaId=${firebaseMetaId}`
        )
      ); // NEED FIREBASEMETAID!
    }
    if (submissionSuccess !== null && !submissionSuccess) {
  ...
  const onFinalCancel = () => {
    let r = window.confirm(
      `Sure you want to cancel? Any conversions / changes will be lost`
    );
    if (r == true) {
      history.push(`/`);
    }
  ...
  const screenToRender = () => {
    ...
    if (!submissionFeedback) {
      return (
        <div
          className="FinalReviewInterfaceContainer Flex Column"
          ...
          <>
            ...
            <div className="FinalReviewControlsContainer Flex Column">
              ...
              <div className="FinalReviewButtonsContainer">
                <button type="button" onClick={onFinalCancel}>
                  Cancel
                </button>
                <button type="button" id="ReviewConfirm" onClick={onFinalClick}>
                  Confirm
                </button>
              </div>
            </div>
