...
function ContributorsSection({ contributors, posterAliasesInState, setPosterAliasesInState }) {
  ...
  const handleUngroup = () => {
    ...
    // ------ NO! more steps needed here. this is too blunt. ----------
  ...
  const handleConfirmAndSaveGroup = () => {
    ...
    */
    ...
    //   const newNameGrouping = { displayName: displayName, aliases: [...namesToGroup] };
    ...
    //   const newNameGrouping = { displayName: displayName, aliases: _.uniq([...namesToGroup, ...targetPosterAliasesObj.aliases]) };
  ...
  const handleRenameClickPoster = (e) => {
    console.log(e.target.name)
    setPosterToRename({ original: e.target.name, newName: null });
  }

  const handleChangeRename = (e) => {
    setPosterToRename({ ...posterToRename, newName: e.target.value });
  }
    ...
    if (editView === 'edit_options') {
      return (
        <div className="EditOptions">
          ...
          <button type="button" onClick={handleUngroup}>Ungroup</button>
        </div>
      );
    };
    ...
    if (editView === 'edit_rename') {
      return (
        <div className="EditRename">
          <h5>Pick a poster to rename</h5>
          {contributors.map((contributor) => {
            // console.log(contributor, '<<<')
            const { poster } = contributor;
            return (
              <div className="CheckBoxOptionCard Flex Row">
                {posterToRename.original !== contributor ?
                  <button type="button" name={poster} onClick={(event) => handleRenameClickPoster(event)}>
                    <span>{poster}</span>
                  </button>
                  : <input type="text" value={posterToRename.newName} onChange={handleChangeRename} />
                }
              </div>
            )
          })}
...
export default ContributorsSectionBACKUP;
