...
export const createSpotifyPlaylist = (
  ...
  return axios({
    ...
    console.log(e);
    return {
      error: { msg: 'Unable to create playlist. Please try again later' },
    };
  });
};
...
export const deleteFirebasePlaylist = async (
  ...
  const deletePlaylistResponse = await axios.delete(
    `${firebaseUrl}/playlists/${firebasePlaylistId}.json?auth=${token}`
  );
  ...
  await axios.delete(
    `${firebaseUrl}/playlistMetas/${metaId}.json?auth=${token}`
  );
  return deletePlaylistResponse.status;
};
...
export const createOrUpdateFirebasePlaylistMetadata = async (
  ...
  if (method === 'PATCH') {
    ...
    const { data } = await axios.get(
      `${firebaseUrl}/playlistMetas.json?orderBy="firebasePlaylistId"&equalTo="${firebasePlaylistId}"&auth=${token}`
    );
  ...
  const { spotifyPlaylistId, spotifyPlaylistName, processedPostsLog } =
    playlistData;
  ...
  return axios({
    ...
    .then((createPlaylist) => {
      if ([200, 201].includes(createPlaylist.status)) {
        ...
        return createOrUpdateFirebasePlaylistMetadata(
          method,
          newFirebasePlaylistId,
          playlistData,
          firebaseUserId,
          token
        )
...
export const getSpotifyPlaylist = async (spotifyPlaylistId, spotifyToken) => {
  ...
  while (offset < totalTracks) {
    ...
    if (response) {
      allTracks = allTracks.concat(response.data.items); // Combine the new tracks with the existing ones
...
export const getYoutubeVideosAndClosestSpotifyMatches = async (
  youtubePosts,
  youtubeApiKey,
  spotifyToken
) => {
  // HANDLE ANY YOUTUBE LINKS
  const youtubeApiBaseURL1 =
    'https://content-youtube.googleapis.com/youtube/v3/videos?id=';
  ...
  if (youtubePosts.length) {
    ...
    const youtubeGetResponses = await Promise.all(
      videoDataQueries.map(
        async (query) =>
          await axios.get(query).catch((e) => {
            console.log(e);
            return e;
          })
      )
    ...
    const spotifySearchQueries = videoDataObjs.map((el) =>
      el?.title
        ? `https://api.spotify.com/v1/search?q=${encodeURIComponent(
            el.title
          )}&type=track&market=GB&limit=10`
        : null
    );
    ...
    */
    ...
    const spotifyGetResponses = await Promise.all(
      spotifySearchQueries.map(async (query) => {
        if (!query) {
          ...
          const axiosReqRetry = await axios
            .get(query, {
              headers: {
    ...
    const closestMatchInEachSpotifySearchResponse = spotifyGetResponses.map(
      (spotiRes, i) => {
        if (!spotiRes) {
        ...
        const correspondingVideoTitle = videoDataObjs[i]
          ? accents.remove(videoDataObjs[i].title)
          : null;
        ...
        const fiveTracksScored = fiveTracksCondensed.map(
          (titleAndArtistsJoined, i) => {
            const scoreSimilarity = (aVideoTitle, aString) => {
              ...
              videoTitleTerms.forEach((term) =>
                aString.split(' ').includes(term) ? count++ : null
              );

              // handle karaoke versions - if 'karaoke' not in vid title, and 'karoke' found in Spoti result title, set similarity = 0
              if (
                !h.stringContainsKaraoke(aVideoTitle) &&
                h.stringContainsKaraoke(aString)
              )
                count = 0;
              if (
                !h.stringContainsAcoustic(aVideoTitle) &&
                h.stringContainsAcoustic(aString)
              )
                count = 0;
              if (
                !h.stringContainsLive(aVideoTitle) &&
                h.stringContainsLive(aString)
              )
                count = 0;
              if (
                !h.stringContainsRemix(aVideoTitle) &&
                h.stringContainsRemix(aString)
              )
                count = 0;
              if (
                !h.stringContainsMix(aVideoTitle) &&
                h.stringContainsMix(aString)
              )
                count = 0;
              if (
                !h.stringContainsEdit(aVideoTitle) &&
                h.stringContainsEdit(aString)
              )
                count = 0;
              if (
                !h.stringContainsExtended(aVideoTitle) &&
                h.stringContainsExtended(aString)
              )
                count = 0;
              if (
                !h.stringContainsVersion(aVideoTitle) &&
                h.stringContainsVersion(aString)
              )
                count = 0;
              if (
                !h.stringContainsCover(aVideoTitle) &&
                h.stringContainsCover(aString)
              )
                count = 0;
              if (
                !h.stringContainsPiano(aVideoTitle) &&
                h.stringContainsPiano(aString)
              )
                count = 0;
              if (
                !h.stringContainsDub(aVideoTitle) &&
                h.stringContainsDub(aString)
              )
                count = 0;
            ...
            const similarity = scoreSimilarity(
              correspondingVideoTitle.toLowerCase(),
              titleAndArtistsJoined.toLowerCase()
            );
            return {
              similarity: similarity,
              trackMeta: titleAndArtistsJoined,
              itemsIndex: i,
            };
          }
        );
        ...
        const highestScore = Math.max(
          ...fiveTracksScored.map((e) => e.similarity)
        );
        const highestScoringCandidates = fiveTracksScored.filter(
          (e) => e.similarity === highestScore
        );
    ...
    const spotifyDataObjs = closestMatchInEachSpotifySearchResponse.map(
      (el, i) => {
        // if (!el) return null;
        const spotifyTrackData = { ...youtubePosts[i], include: true };
        if (!el) return spotifyTrackData;
        spotifyTrackData.artists =
          el?.artists.map((artist) => artist.name) || null;
...
export const getSpotifyTrackData = async (spotifyPosts, spotifyToken) => {
  ...
  const getTracksResponsesFlattened = spotifyGetTracksResponses.reduce(
    (acc, e, i) => {
        ...
        e.data.tracks.forEach((obj) => {
          ...
          if (obj) {
            acc.push({
              ...
              thumbnailSmall:
                obj.album.images[2].url ||
                obj.album.images[1].url ||
                obj.album.images[0].url,
              thumbnailMed:
                obj.album.images[1].url ||
                obj.album.images[0].url ||
                obj.album.images[2].url,
  ...
  const spotifyPostsCompleteData = spotifyPosts.map((obj, i) => ({
    ...obj,
    ...getTracksResponsesFlattened[i],
  }));
...
export const getGenresForSpotifyTracks = async (tracksArr, spotifyToken) => {
  ...
  tracksArr.forEach((track) => {
    track.artistIDs.forEach((artistID) =>
      postArtists.push({ postId: track.postId, artistID, genres: [] })
    );
  });

  const postArtistsMax50Each = _.chunk(postArtists, 50);

  const spotifyGetArtistsQueries = postArtistsMax50Each.map(
    (arrayOfMa50PostArtists) => {
      const max50IDsJoined = arrayOfMa50PostArtists
        .map((e) => e.artistID)
        .join(',');
  ...
  const getArtistsResponsesFlattened = spotifyGetArtistsResponses.reduce(
    (acc, e, i) => {
      if (/^5\d{2}$/g.test(e.response?.status)) {
        console.log('🚨');
        // 500 error - server error. We want to stop the Submission process overall
  ...
  const tracksArrPlusGenres = tracksArr.map((trackObj) => {
    const genresForAllTrackArtists = postArtistsPlusGenres
      .filter((e) => e.postId === trackObj.postId)
      .reduce((acc, postArtistObj) => {
...
export const postToSpotifyPlaylist = async (
  targetPlaylistID,
  spotifyToken,
  trackIDs,
  input
  ...
  const spotifyPostTracksResponses = await makePostTracksRequests(
    subArraysMax100Each
  );
...
export const updatePlaylistMetaLookup = async (
  lookupInState,
  metaId,
  token
) => {
...
export const searchForUsersPlaylistByName = async (
  ...
  if (status !== 200 || !data) {
    ...
    if (playlistWithThisNameAlreadyExists) {
      result.error = {
        msg: 'You have already created this playlist today. Please delete it on Spotify before trying again',
      };
    } else {
      return result;
    }
...
export const createPosterPlaylist = async (
  ...
  const searchResponse = await searchForUsersPlaylistByName(
    newPosterPlaylistName,
    spotifyToken,
    spotifyUserId
  );
  ...
  const createResponse = await createSpotifyPlaylist(
    spotifyUserId,
    spotifyToken,
    newPosterPlaylistName
  );
  ...
  const addTracksResponse = await postToSpotifyPlaylist(
    newPlaylistId,
    spotifyToken,
    posterTrackIDs
  );
...
export const getSpotifyAlbumsData = async (albumPosts, spotifyToken) => {
  ...
  const getAlbumsResponsesFlattened = spotifyGetAlbumsResponses.reduce(
    (acc, e) => {
        ...
        e.data.albums.forEach((obj) => {
          ...
          if (!obj) {
            ...
            acc.push({
              artists: obj.artists.map((artist) => artist.name),
              thumbnailSmall:
                obj.images[2].url || obj.images[1].url || obj.images[0].url,
              thumbnailMed:
                obj.images[1].url || obj.images[0].url || obj.images[2].url,
              title: obj.name,
  ...
  const albumPostsCompleteData = albumPosts.map((obj, i) => {
    // 🚦 🚦 🚦
    if (
      getAlbumsResponsesFlattened[i] !== null &&
      getAlbumsResponsesFlattened[i] !== 500
    )
      return { ...obj, ...getAlbumsResponsesFlattened[i] }; // 🚦 🚦 🚦
    return { ...obj }; // 🚦 🚦 🚦
  }); // 🚦 🚦 🚦
...
export const getSpotifyPlaylistsData = async (playlistPosts, spotifyToken) => {
  ...
  const getPlaylistsResponsesFlattened = spotifyGetPlaylistResponses.reduce(
    (acc, e, i) => {
      // 🚦 🚦 🚦
      // console.log(e.response)
      if (/^5\d{2}$/g.test(e.response?.status)) {
  ...
  const playlistPostsCompleteData = playlistPosts.map((obj, i) => {
    // 🚦 🚦 🚦
    if (
      getPlaylistsResponsesFlattened[i] !== null &&
      getPlaylistsResponsesFlattened[i] !== 500
    )
      return { ...obj, ...getPlaylistsResponsesFlattened[i] }; // 🚦 🚦 🚦
    return { ...obj }; // 🚦 🚦 🚦
  }); // 🚦 🚦 🚦
  return playlistPostsCompleteData; // 🚦 🚦 🚦
};

export const exportStatsPage = async (
  firebasePlaylistObj,
  firebaseMetaObj,
  spotifyPlaylistData,
  token
...
export const deleteAccountAndPlaylists = async (
  userPlaylistMetas,
  firebaseUserId,
  firebaseToken
) => {
  ...
  const deletePlaylistsAndMetaResponses = await Promise.all(
    firebasePlaylistIdsAndMetaIds.map(
      async (obj) =>
        await deleteFirebasePlaylist(
          obj.firebasePlaylistId,
          obj.metaId,
          firebaseToken
        ).then((res) => res)
    )
  ...
  const deletePlaylistsAndMetaResponsesAll200Or204 =
    deletePlaylistsAndMetaResponses.every((e) => e === 200 || e === 204);

  if (!deletePlaylistsAndMetaResponsesAll200Or204) return null;

  const correspPublicStats = await axios.get(
    `${firebaseUrl}/publicStats.json?orderBy="firebaseUserId"&equalTo="${firebaseUserId}"&auth=${firebaseToken}`
  );
  ...
  if (correspPublicStats.status === 200) {
    ...
    const deletePublicStatsQueries = publicStatsIds.map(
      (publicStatsId) =>
        `${firebaseUrl}/publicStats/${publicStatsId}.json?auth=${firebaseToken}`
    );

    const deletePublicStatsResponses = await Promise.all(
      deletePublicStatsQueries.map(
        async (query) =>
          await axios.delete(query).catch((e) => {
            console.log(e);
            return e;
          })
      )
    ...
    const deletePublicStatsResponsesAll200Or204 =
      deletePublicStatsResponses.every((e) => [200, 204].includes(e.status));
    ...
    const userDeleteResponse = await axios
      .post(
