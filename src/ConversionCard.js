...
function ConversionCard({
    ...
    if (!spotifyMatch.spotifyTrackID) {
      return (
        <div className="SegmentWrapper SpotifySegmentWrapper">
          <div
            className="SpotifyNullSegment Flex Column"
            onClick={(e) => e.stopPropagation()}
            key={`matchCard-${index}`}
          >
            <p>
              No match found on Spotify - no track will be added to the Spotify
              playlist for this
            </p>
          </div>
        </div>
      ...
      <div className="SegmentWrapper SpotifySegmentWrapper">
        <div className="SpotifySegment Flex Row">
          <div className="ConversionSpotifyThumbnailContainer Flex Column">
            <img
              className="ConversionSpotifyThumbnail"
              src={thumbnailMed}
          ...
          <div className="ConversionSpotifyInfoContainer Flex Column">
            ...
            <div className="SpotifyArtists Flex Column">
              <span className="CurtailText Curtail2">
                {artists.join(', ')}{' '}
              </span>
            </div>
          </div>
        ...
        <a
          href={`https://open.spotify.com/track/${spotifyMatch.spotifyTrackID}`}
          target="_blank"
        />
      </div>
    );
  ...
  const conversionCard = () => {
    ...
    console.log(include);
    const undoEnabled =
      spotifyMatch.spotifyTrackID !== spotifyMatches[index].spotifyTrackID;
    return (
      <div className="YTC-Wrapper Flex Row">
        <div className="ChangedAsterisk">
          {undoEnabled && include ? (
            <FontAwesomeIcon icon={faAsterisk} pointerEvents="none" />
          ) : null}
        </div>

        <div
          className={`YTC-ButtonsMenuContainer Flex Column ButtonsMenuContainer-Include-${include}`}
        >
          <button
            className={`CC-Side-Top2-${include}`}
            onClick={() => handleUndoMatchChanges(index)}
          ...
          <button
            className={`CC-Side-Top2-${include}`}
            onClick={() => handleChangeMatch(spotifyMatch, index)}
            disabled={!include}
          >
            <FontAwesomeIcon icon={faSearch} pointerEvents="none" />
          </button>
          <button
            className={`CC-Side-Exclude-${include}`}
            onClick={() =>
              include ? handleExcludeMatch(index) : handleIncludeMatch(index)
            }
          >
            {include ? (
              <FontAwesomeIcon
